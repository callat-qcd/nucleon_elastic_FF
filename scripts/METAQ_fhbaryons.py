...
parser.add_argument('--cleanup',      default=False, action='store_true',
                    help=             'remove src, prop and fhprop files? [%(default)s]')
...
for c in cfgs_run:
    ...
    if os.path.exists(cfg_file):
        ...
        for s0 in srcs[c]:
            ...
            strange_prop = c51.names['prop'] % params
            ...
            if not os.path.exists(fh_baryons_file):
                ...
                for fh in ['A3','V4']:
                    ...
                    if not os.path.exists(fh_prop_files[fh]):
                        have_fh_props = False
                ''' does incoming prop exist? '''
                have_prop = os.path.exists(prop_file) and os.path.exists(strange_prop_file)

                if have_fh_props and have_prop:
                    metaq = fh_baryons_name +'.sh'
                    ...
                    if not t_e or (args.o and not t_w):
                        xmlini = params['xml'] +'/'+ fh_baryons_name+'.ini.xml'
                        fin    = open(xmlini,'w')
                        ...
                        params['OBJ_ID']    = strange_prop
                        params['LIME_FILE'] = strange_prop_file
                        fin.write(xml_input.qio_read % params)
                        params.update({'SMEARED_PROP':strange_prop+'_SS', 'PROP_NAME':strange_prop})
                        fin.write(xml_input.shell_smearing % params)
                    ...
                    if args.verbose:
                        ...
                        if not os.path.exists(strange_prop_file):
                            print('missing prop',strange_prop_file)
                ...
                if args.cleanup:
                    # check if hyperspec exists also
                    ...
                    if os.path.exists(hyperspec_file):
                        print('CLEANUP: Deleting')
                        # we can delete the src prop prop_strange fh_prop files
                        ...
                        if os.path.exists(src_file):
                            ...
                            os.remove(src_file)
                        for fh in fh_props:
                            if os.path.exists(fh_prop_files[fh]):
                                print(fh_props[fh])
                                os.remove(fh_prop_files[fh])
