...
if any(host in hn for host in ["oslic", "pascal"]):
    ...
    root = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "production")
    machine = "default"
    env = ""
    bind_dir = ""
...
def ensemble(params):
    ...
    params["formfac_4D_tslice"] = (
        params["prod"] + "/" + "formfac_4D_tslice" + "/" + params["CFG"]
    )
...
names[
    "seqsrc"
] += "_gf%(FLOW_TIME)s_w%(WF_S)s_n%(WF_N)s_M5%(M5)s_L5%(L5)s_a%(alpha5)s_mq%(MQ)s"
