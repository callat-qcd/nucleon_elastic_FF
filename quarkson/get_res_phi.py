...
parser.add_argument(
    "-v", default=True, action="store_const", const=False, help="verbose? [%(default)s]"
)
            ...
            if os.path.getsize(prop_xml) > 0:
                with open(prop_xml) as f:
                    ...
                    if data[-1] == "</propagator>":
                        ...
                        shutil.move(
                            prop_xml, params["corrupt"] + "/" + prop_xml.split("/")[-1]
                        )
        else:
            f_good = False
        if not f_good:
            ...
            f5 = h5.open_file(
                data_dir + "/" + ens_s + "_" + no + "_srcs" + src_ext + ".h5", "a"
            )
            ...
            try:
                ...
                f5.create_group(
                    "/" + val_p + "/dwf_jmu/mq" + mq, "pseudo_pseudo", createparents=True
                )
                f5.flush()
            except:
                pass
            ...
            get_data = False
            if (
                src not in f5.get_node(mpdir)
                or src not in f5.get_node(ppdir)
                or src not in f5.get_node(phi_dir)
            ):
                get_data = True
            if args.o and (
                src in f5.get_node(mpdir)
                or src in f5.get_node(ppdir)
                or src in f5.get_node(phi_dir)
            ):
                get_data = True
            if get_data:
                ...
                while not have_data:
                    if (
                        f[l].find("<DWF_MidPoint_Pseudo>") > 0
                        and f[l + 3].find("<DWF_Psuedo_Pseudo>") > 0
                    ):
                        corr_mp = (
                            f[l + 1].split("<mesprop>")[1].split("</mesprop>")[0].split()
                        )
                        corr_pp = (
                            f[l + 4].split("<mesprop>")[1].split("</mesprop>")[0].split()
                        )
                        nt = len(corr_mp)
                    ...
                    if f[l].find("<prop_corr>") > 0:
                        corr = (
                            f[l].split("<prop_corr>")[1].split("</prop_corr>")[0].split()
                        )
