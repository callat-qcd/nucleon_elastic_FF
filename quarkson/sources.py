    ...
    if ens == "a15m135XL":
        if any(host in hn for host in ["lassen"]):
            ...
            if stream in ["c", "d"]:
                ...
                print(
                    "summit is not set to run stream %s yet with srcs 0-7 x 1" % stream
                )
                sys.exit()
    return params
...
def xXyYzZtT(posn):
    if len(posn) is 4:
        ...
        print(
            sys.argv[0], ": Position must be a four dimensional array to be stringified."
        )
        exit()
...
def antipode(nl):
    ...
    def generator(o):
        ...
        [x0, y0, z0, t0] = o
        dx = int(nl / 2)
        a = {"A": modByLattice(nl)([x0 + dx, y0 + dx, z0 + dx, t0])}
        return a
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--src", type=str, help="pass a specific src if desired, xXyYzZtT"
    )
    parser.add_argument(
        "--smr", type=str, help="pass a specific smearing if desired, wW_nN"
    )
    parser.add_argument(
        "--status",
        type=str,
        help='A new status for this source.  Status can be "None", ""',
    )
    parser.add_argument(
        "-p",
        "--print",
    ...
    parser.add_argument(
        ...
        type=str,
        default="1a",
        help="seed used in src generation [%(default)s]",
    )
    parser.add_argument(
        "--nl", type=int, default=32, help="spatial volume [%(default)s]"
    )
    parser.add_argument(
        "--nt", type=int, default=96, help="spatial time extent [%(default)s]"
    )
    parser.add_argument(
        "--t_shifts",
        nargs="+",
    ...
    if args.generator == "oa":
        ...
        print(
            "we only support print testing of o, a, oa at the moment - add more if you like"
        )
        sys.exit()
